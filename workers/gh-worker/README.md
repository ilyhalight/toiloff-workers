...
Any-side (Client/Server) worker that collects stars and commit stats from Github and sends them to [backend](https://github.com/ilyhalight/toiloff-backend).

Worker will use pagination if you have too many repos...
...
- `GH_CLASSIC_TOKEN` - Classic (!) token with `public_repo`, `read:user` scopes

## Install

You can download `gh-worker` for linux from [Releases](https://github.com/ilyhalight/toiloff-workers/releases).
...
3. Navigate to the `workers/gh-worker` directory
